    ...
    if (existingPerson) {
      if (confirm(`${newName} is already added to phonebook, replace the old number with a new one?`)) {
        personService
          ...
          .catch(error => {
            notificate(`Information of ${existingPerson.name} has already been removed from server`, true)
...
const Persons = ({ keyword, persons, handleDelete}) => {
  ...
  return matches.map(p => 
    <div key={p.name}>
      <p>{p.name} {p.number} <DeleteButton handleDelete={handleDelete} id={p.id} /></p>
    </div>
    )
}
